        ...
        if($coupon) {
            ...
            if (strtotime($coupon->expiry_date) <= strtotime($current_dttm->toDateTimeString())) {
                if ($sub_total >= $coupon->amount) {
                    if ($coupon->coupon_type == 'multi_time') {
                        $coupon_user = CouponUser::query()->where(['coupon_id' => $coupon->id , 'user_id' => auth()->id()])->firstOrFail();

                        if($coupon_user){
                            return  js_response(null, "You can only use this Coupon Code Once.", false);
                        }
                        else{
